...
- Finish to fix all (known) remote memory access issues. Some of them
  probably remain.
*** Questions
 - Currently a quite dumb version approach si sued where most of the
   data is read from the other process memory on demande. Will the
   ...
   MCed side).
*** Work Planed
**** TODO Safety algorithm on the MCer side
...
restoration): the communication if based on message passing.
...
**** TODO Implement the liveness in multi-process
